...
class SellAllCommand extends Command implements PluginOwned {
    ...
    public function __construct(Plugin $plugin) {
        parent::__construct("sellall", "Sell all items of the same type in your inventory");
        $this->plugin = $plugin;
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool {
        ...
        foreach ($sellableItems as $itemData) {
            if (is_array($itemData) && isset($itemData["name"]) && is_string($itemData["name"])) {
                ...
                $itemInHand = $inventory->getItem($parsedItem->getId(), $parsedItem->getMeta());

                if ($itemInHand->getId() === VanillaItems::AIR) {
                    continue; // Skip if the player doesn't have the item in their inventory.
                }
        ...
        if ($totalEarnings > 0) {
            $this->economyManager->addMoney($sender, $totalEarnings, function(bool $success) use ($sender, $totalEarnings) {
                if ($success) {
                    $sender->sendMessage("§l§a(§f!§a) §r§fYou have sold all eligible items of the same type in your inventory for §e$" . $totalEarnings . "§f!");
                } else {
                    $sender->sendMessage("§l§c(§f!§c) §r§fFailed to add money. Please check your economy plugin configuration.");
                }
            });
        } else {
            $sender->sendMessage("§l§c(§f!§c) §r§fNo eligible items of the same type to sell in your inventory.");
        }
