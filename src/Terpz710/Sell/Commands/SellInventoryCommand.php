...
class SellInventoryCommand extends Command implements PluginOwned {
    private $plugin;
    private $itemsConfig;
    private $economyManager;

    public function __construct(Plugin $plugin) {
        parent::__construct("sellinventory", "Sell all items in your inventory");
        $this->plugin = $plugin;
        $this->setPermission("sell.sellinventory");
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool {
        ...
        if ($totalEarnings > 0) {
            $this->economyManager->addMoney($sender, $totalEarnings, function(bool $success) use ($sender, $totalEarnings) {
                if ($success) {
                    $sender->sendMessage("§l§a(§f!§a) §r§fYou have sold all eligible items in your inventory for §e$" . $totalEarnings . "§f!");
                } else {
                    $sender->sendMessage("§l§c(§f!§c) §r§fFailed to add money. Please check your economy plugin configuration.");
                }
            });
        } else {
            $sender->sendMessage("§l§c(§f!§c) §r§fNo eligible items to sell in your inventory.");
        }
